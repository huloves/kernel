	...
	.w0:
		mov al,0x0a ;阻断NMI。通常是不必要的
		or al,80
		out 0x70,al
		in al,0x71	;读寄存器A
		test al,0x80	;测试第7位UIP
		jnz .w0		;以上到吗对于更新周期结束中断来说
					;是不必要的
		xor al,al
		or al,0x80
		out 0x70,al
		...
		out 0x70,al
		in al,0x71	;第一下RTC的寄存器C，否则只发生
					;依次中断，此处不考虑闹钟和周期性中断的情况
		mov ax,0xb800
		mov es,ax
	...
	start:
		...
		out 0x71,al	;更新后中断，BCD码，24小时制

		mov al,0x0c
		out 0x70,al
		in al,0x71	;读RTC寄存器C，复位未决的中断信号

		in al,0xa1	;读8259从片的IMP寄存器
