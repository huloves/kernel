...
{
    uint8_t rollback_step = 0;   //用于操作失败时返回回滚各资源状态
    ...
    if(inode_no != -1) {
        ...
        //判断是否把pathname的各层目录都访问到了，即是否在某个中间目录就失败了
        if(pathname_depth != path_searched_depth) {   //说明并没有访问到全部的路径，中间某个目录是不存在的
    ...
    uint32_t block_bitmap_idx = 0;   //记录block对应与block_bitmap中的索引
    int32_t block_lba = -1;
    //为目录分配一个块，用来写入目录.和..
...
/*在inode编号为p_inode_nr的目录中查找inode编号为c_inode_nr的子目录的名字，名字存入path*/
static int get_child_dir_name(uint32_t p_inode_nr, uint32_t c_inode_nr, char* path, void* io_buf)
{
    struct inode* parent_dir_inode = inode_open(cur_part, p_inode_nr);
    //填充all_blocks，将该目录所占扇区全部地址写入all_blocks
...
char* sys_getcwd(char* buf, uint32_t size)
{
    ...
    //当child_inode_nr为根目录的inode编号0时停止，即已查看完根目录中的目录项
    ...
    //full_path_recerse中的路径是反着的，要将其反置
...
int32_t sys_chdir(const char* path)
{
    ...
    if(inode_no != -1) {
        if(searched_record.file_type == FT_DIRECTORY) {
            ...
            printk("sys_chdir: %s id regular file or other!\n");
        }
    }
    dir_close(searched_record.parent_dir);
    return ret;
