...
static struct page * follow_page(struct mm_struct *mm, unsigned long address, int write) 
{
	...
	pgd = pgd_offset(mm, address);   //获取address处的PGD并保证它存在且有效
	...
	pte = *ptep;
	if (pte_present(pte)) {   //判断PTE当前是否存在
		if (!write ||   //指定要进行写操作，则检查PTE是否有写权限。若有写权限设置PTE为脏
		    (pte_write(pte) && pte_dirty(pte)))
			return pte_page(pte);   //若PTE存在并且具有权限，就可以返回PTE对应的struct page结构
	}

out:
	return 0;   //返回0表明address没有相应的struct page
}
