...
{
	struct address_space *mapping = page->mapping;   //获取页面的address_space，如果它存在的话
...
void ___wait_on_page(struct page *page)
{
	...
	DECLARE_WAITQUEUE(wait, tsk);   //初始化当前任务的等待队列

	add_wait_queue(waitqueue, &wait);   //将进程添加到page_waitqueue()返回的等待队列中
	do {
...
{
	wait_queue_head_t *waitqueue = page_waitqueue(page);
	ClearPageLaunder(page);   //Clears the launder bit because I/O has now completed on the page
	smp_mb__before_clear_bit();   //一个memory block operation，必须在执行多个处理器可见的位操作之前调用
	if (!test_and_clear_bit(PG_locked, &(page)->flags))
		BUG();
	smp_mb__after_clear_bit();   //完成memory block operation
	 ...
	 */
	if (waitqueue_active(waitqueue))   //若有进程在该页面的等待队列中，则唤醒它们
 ...
 * to get it..
 */
static void __lock_page(struct page *page)   //TryLockPage()失败后调用，进入这个页面的一个等待队列，并在其中睡眠，直到可以获得锁
{
	wait_queue_head_t *waitqueue = page_waitqueue(page);   //决定该页面属于zone->wait_table表中的哪一个队列
	struct task_struct *tsk = current;
	DECLARE_WAITQUEUE(wait, tsk);   //初始化进程的等待队列

	add_wait_queue_exclusive(waitqueue, &wait);   //将进程加入到page_waitqueue()返回的等待队列中
	for (;;) {   //一直循环，直到获得锁
		set_task_state(tsk, TASK_UNINTERRUPTIBLE);   //将进程设置为不可中断睡眠，一直睡眠到被显示的唤醒
		if (PageLocked(page)) {   //若页面仍被锁定，调用sync_page()与backing storage同步
			sync_page(page);
			schedule();   //调度其他任务执行
		}
		if (!TryLockPage(page))   //尝试锁住页面。若成功则跳出循环，否则再次在队列中睡眠
			break;
	}
	__set_task_state(tsk, TASK_RUNNING);   //将进程状态设置为TASK_RUNNING
	remove_wait_queue(waitqueue, &wait);   //并从等待队列中移除该进程，该函数返回时，进程获得了页面的锁
}
 ...
 */
void lock_page(struct page *page)   //尝试锁住页面，若页面无法被锁住，就会导致进程睡眠知道页面可以被锁住
{
	if (TryLockPage(page))   //PG_locked，若该位已经被清除，函数立即返回，因为页面已经被锁住
		__lock_page(page);   //调用该函数使进程睡眠
}
