...
/* Change Log
 * 5.6.10	11/23/04
 * - Withdraw the fixed Rx buffer changes added to this driver in 5.6.4. 
 * - This causes system instability that is not fully understood yet.
 * 5.6.9        11/16/04
 * - Include IE components 9.3.0.7
 * 5.6.8 	11/11/04
 * - Fixed ethtool_phys_id to use msleep_interrupible
 * - Fixed msleep_interruptible to interpret input param correctly
 * - Fixed setup_rctl to set RCTL:RX_BUFFER_SIZE bits correctly for 
 *   MTU >= 4096
 * 5.6.7 	11/09/04
 * - Fixed default ITR setting for 2.6.x kernels
 * - Include new shared code 
 *     fix 'unused e1000_hw.c:663: warning: unused variable `reg_data
 * 5.6.6 	11/08/04
 * - Fixed kcompat pci_{save|restore}_state
 * 5.6.5 	11/01/04
 * - Remove redundant logic (as a result of fixed sized rx-buffers)
 *   from e1000_setup_rctl.
 * - Integrate the fix for kernel panic when the interface is brought down 
 *   while the NAPI enabled driver is under stress 
 * - Fix ethtool diagnostics -- link, loopback and interrupt tests
 * - Enabling NETIF_F_SG without checksum offload is illegal - 
     John Mason <jdmason@us.ibm.com>
 * 5.6.4        10/27/04
 * - Use fixed sized rx-buffers irrespective of the MTU size. MTUs 8K 
 *   and above will use 4K buffers while those below use 2K buffers.
 * 5.6.3        10/26/04
...
#endif
char e1000_driver_version[] = "5.6.10.1"DRIVERNAPI;
